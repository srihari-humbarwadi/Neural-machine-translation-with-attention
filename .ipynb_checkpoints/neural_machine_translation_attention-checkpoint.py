...
def preprocess_en(en):
    ...
    padded_en = pad_sequences(
        encoded_en, maxlen=max_sequence_len, padding='post', truncating='post')
    return padded_en


def preprocess_fr(fr):
    encoded_fr = fr_tk.texts_to_sequences(fr)
    padded_fr = pad_sequences(
        encoded_fr, maxlen=max_sequence_len, padding='post', truncating='post')
    return padded_fr


def preprocess_text(en, fr):
    return preprocess_en(en.numpy().decode()),
    preprocess_fr(fr.numpy().decode())
...
def training_step(en, fr):
    with tf.GradientTape() as Tape:
        ...
        encoder_output, encoder_hidden_state = encoder(
            en, encoder_init_state, training=True)
        ...
        for word_idx in range(1, max_sequence_len):
            ...
            logits, decoder_hidden, attention_weights = decoder(
                current_word, encoder_output, decoder_hidden, training=True)
...
for epoch in range(1, epochs + 1):
    ...
    ep_loss = []
    ep_acc = []
    progbar = tf.keras.utils.Progbar(target=num_steps,
                                     stateful_metrics=[
                                         'curr_loss', 'curr_accuracy'],
                                     unit_name='batch')
...
def translate_sentence(sentence):
    ...
    for word_idx in range(1, max_sequence_len):
        logits, decoder_hidden, attention_weights = decoder(
            current_word, enc_out, decoder_hidden)
